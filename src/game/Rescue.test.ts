...
describe('Rescue', () => {
  ...
  it('starts empty-handed with an animal waiting and no score', () => {
    expect(rescue.carrying).toBe(false)
    expect(rescue.animalWaiting).toBe(true)
    expect(rescue.score).toBe(0)
  })

  it('picks the animal up on landing at the pickup pad', () => {
  ...
  it('stays parked on the pickup pad without picking up twice', () => {
    rescue.landedOn(pickup)
    expect(rescue.landedOn(pickup)).toBeNull()
    expect(rescue.carrying).toBe(true)
  })

  it('scores on delivering to the rescue pad', () => {
    rescue.landedOn(pickup)
    expect(rescue.landedOn(rescuePad)).toBe('delivered')
    expect(rescue.score).toBe(1)
    expect(rescue.carrying).toBe(false)
  })
  ...
  it('offers another animal so the loop can be flown repeatedly', () => {
    for (let trip = 1; trip <= 3; trip++) {
      expect(rescue.landedOn(pickup)).toBe('picked-up')
      expect(rescue.landedOn(rescuePad)).toBe('delivered')
      expect(rescue.score).toBe(trip)
    }
  })
})
